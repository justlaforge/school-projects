...
int main(int argc, char **argv){
    ...
    allocate2d_3(&outData, rows+1, cols);                  // Allocate space for output data

    // perform row checksum
    cs_cols(data, rows, cols, outData);
    
    // insert 1d array into file
    write_memory_to_file2d(outData, rows+1, cols, output);

	if(pFlag){
		print_memory(&outData, rows+1, cols);
	}
