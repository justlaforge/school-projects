...
int main(int argc, char **argv){
    ...
    allocate2d_3(&outData, rows, cols+1);                  // Allocate space for output data

    // perform row checksum
    cs_rows(data, rows, cols, outData);

    write_memory_to_file2d(outData, rows, cols+1, output);
    
	if(pFlag){
		print_memory(&outData, rows, cols+1);
	}
